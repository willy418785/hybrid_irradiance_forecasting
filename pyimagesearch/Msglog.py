...
#
def LogInit(name, filename, debuglevel = logging.INFO, log = True,stdout=False):
    ...
    if stdout == True:
        sd = logging.StreamHandler(sys.stdout)
        sd.setLevel(debuglevel)
    ...
    if log == True:
        ...
        fh = logging.handlers.TimedRotatingFileHandler(Path(filename + ".log"), when='midnight')
